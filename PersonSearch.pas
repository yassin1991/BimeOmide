...
uses
  ...
  Vcl.StdCtrls, Vcl.Buttons,NewFile;

type
  TPersonSearchForm = class(TForm)
    DBGridEh1: TDBGridEh;
    BitBtn1: TBitBtn;
    BitBtn2: TBitBtn;
...
var
  PersonSearchForm: TPersonSearchForm;

implementation

{$R *.dfm}

uses FileFind, FindPerson, PersonEdit;

procedure TPersonSearchForm.BitBtn1Click(Sender: TObject);
var
...
begin
   ...
   NewFileForm.PersonComboBox.Text := DBGridEh1.Fields[1].AsString+' '+DBGridEh1.Fields[2].AsString+' کد '+DBGridEh1.Fields[0].AsString;
   PersonSearchForm.Close;
end
else
begin
...
procedure TPersonSearchForm.DBGridEh1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if (key = VK_F10) then
begin
...
procedure TPersonSearchForm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TPersonSearchForm.PersonEditing();
begin
